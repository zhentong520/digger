...
* @Description: 排列5 开奖号码统计
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月5日 上午10:57:32 
*
 */
@Service
public class Pailie5_DrawCodeStatistics implements DrawCodeStatisticsHandler {

	private static final Logger logger=Logger.getLogger(Pailie5_DrawCodeStatistics.class);
	...
	public Pailie5_DrawCodeStatistics() {
	...
	public void setDrawCodeStatistics(Integer lotCode){
		...
		//当年所有记录
		List<AppLotHistory> lots_currYear=lotHistoryService.getNewListByCountAndLotCode(QuartzHandlerType.PAI_LIE_5.getCode(), null,Calendar.YEAR);
		// 前100条
		List<AppLotHistory> lots_100=lotHistoryService.getNewListByCountAndLotCode(QuartzHandlerType.PAI_LIE_5.getCode(), 100,Calendar.YEAR);
		...
		Map<Integer, Integer> firstNumberMissCountMap=null;// 万位初始统计
		Map<Integer, Integer> secondNumberMissCountMap=null;// 千位初始统计
		Map<Integer, Integer> thirdNumberMissCountMap=null;// 百位初始统计
		Map<Integer, Integer> fourthdNumberMissCountMap=null;// 十位初始统计
		Map<Integer, Integer> fifthNumberMissCountMap=null;// 个位初始统计
		...
		if (AssertValue.isNotNull(lot)) {
			...
			if (AssertValue.isNotNull(statistic)) {
				...
				// 十
				fourthdNumberMissCountMap= JSONObject.parseObject(statistic.getFourthArea(), HashMap.class) ;
				paramMaps.add(fourthdNumberMissCountMap);
				// 个
				fifthNumberMissCountMap = JSONObject.parseObject(statistic.getFifthArea(), HashMap.class);
				paramMaps.add(fifthNumberMissCountMap);
			}else {
		...
		if (!AssertValue.isNotNullAndNotEmpty(statisticList)) {
			logger.info(lotCode+" --->>>排列5走势图统计结果列表数量："+statisticList.size());
			return;
		}
	...
	* @throws
	...
	private List<LotDrawCodeStatistic> setFucai3DStatistic(List<AppLotHistory> list,int rows,List<Map<Integer, Integer>> params) {
		...
		// 得到彩种类型
		String name=QuartzHandlerType.PAI_LIE_5.name();
		...
		for (int i = 0; i < params.size(); i++) {
			List<LotDrawCodeStatistic> data=statisticHandler.setStatisticCommonArea(name, list, 0, 9, beginPos, 1, groupIndex, params.get(i),rows);
