...
* @Description: 排列3 开奖号码统计
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月5日 上午10:57:32 
*
 */
@Service
public class Pailie3_DrawCodeStatistics implements DrawCodeStatisticsHandler {

	private static final Logger logger=Logger.getLogger(Pailie3_DrawCodeStatistics.class);
	...
	public Pailie3_DrawCodeStatistics() {
	...
	public void setDrawCodeStatistics(Integer lotCode){
		...
		//当年所有记录
		List<AppLotHistory> lots_currYear=lotHistoryService.getNewListByCountAndLotCode(QuartzHandlerType.PAI_LIE_3.getCode(), null,Calendar.YEAR);
		// 前100条
		List<AppLotHistory> lots_100=lotHistoryService.getNewListByCountAndLotCode(QuartzHandlerType.PAI_LIE_3.getCode(), 100,Calendar.YEAR);
		...
		if (AssertValue.isNotNull(lot)) {
			...
			if (AssertValue.isNotNull(statistic)) {
				...
				// 所有开奖号码并列统计
				Map<Integer, Integer> allNumberMissCountMap= JSONObject.parseObject(statistic.getThirdArea(), HashMap.class) ;
		...
		//保存统计结果
		List<LotDrawCodeStatistic> statisticList = this.setPailie3Statistic(statisticLots, currentRows, paramMaps);
		if (!AssertValue.isNotNullAndNotEmpty(statisticList)) {
			logger.info(lotCode+" --->>>排列3走势图统计结果列表数量："+statisticList.size());
			return;
		}
		...
		// 获取其它列表统计20，50
		this.setPailie3Statistic(statisticLots, 20, paramMaps);
		this.setPailie3Statistic(statisticLots, 50, paramMaps);
		if (currentRows==100) {
			//this.setPailie3Statistic(statisticLots, 0, paramMaps);	
			this.setPailie3Statistic(lots_currYear, 0, paramMaps);	
		}else {
			this.setPailie3Statistic(statisticLots, 100, paramMaps);	
		}
	...
	* @throws
	 */
	private List<LotDrawCodeStatistic> setPailie3Statistic(
			List<AppLotHistory> list,int rows,List<Map<Integer, Integer>> params) {

		// 得到彩种类型
		String name=QuartzHandlerType.PAI_LIE_3.name();
		...
		//List<LotDrawCodeStatistic> firstData=statisticHandler.setStatisticCommonArea(name, list, 0, 9, 1, 1, 1, first_numberMissCountMap,rows);
