...
/**
 * 
* @ClassName: Aozhouxingyun8_QuartzHandler 
* @Description: 定时获取澳洲幸运8 最新开奖记录执行服务类
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月1日 下午2:04:18 
*
 */
@Service
public class Aozhouxingyun8_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Aozhouxingyun8_QuartzHandler.class);
	private LotHistoryService lotHistoryService;
	private LotNewService lotNewService;
	private GrabOpenResultService<?> grabResult;

	public Aozhouxingyun8_QuartzHandler() {
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10011  澳洲幸运8  彩票代码
			List<AppLotHistory> lotList=grabResult.getJingwaiLotHistoryByLotCode(QuartzHandlerType.AO_ZHOU_XING_YUN_8.getCode());
			// 有新数据
			...
			logger.info("获取 澳洲幸运8  新数据成功，下次执行时间：" + CommUtil.formatLongDate(execDate));
		} catch (Exception e) {
			logger.info("获取 澳洲幸运8  新数据异常，下次执行时间：" + CommUtil.formatLongDate(execDate));
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.AO_ZHOU_XING_YUN_8.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.AO_ZHOU_XING_YUN_8, info, execDate);
		}
	...
	* @throws
	...
	public QuartzHandlerType getType() {
		return QuartzHandlerType.AO_ZHOU_XING_YUN_8;
	}

	@Override
	public String getDescription() {
		return QuartzHandlerType.AO_ZHOU_XING_YUN_8.getName();
	}
