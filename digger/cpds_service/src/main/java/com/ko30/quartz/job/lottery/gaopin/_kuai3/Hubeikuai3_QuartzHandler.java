...
/**
 * 
* @ClassName: Hubeikuai3_QuartzHandler 
* @Description: 定时获取 湖北快3 最新开奖记录执行服务类
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月1日 下午2:04:18 
*
 */
@Service
public class Hubeikuai3_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Hubeikuai3_QuartzHandler.class);
	...
	public Hubeikuai3_QuartzHandler() {
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10032  湖北快3  彩票代码
			List<AppLotHistory> lotList=grabResult.getGaopinLotHistoryByLotCode(QuartzHandlerType.HU_BEI_KUAI_3.getCode());
			// 有新数据
			if (AssertValue.isNotNullAndNotEmpty(lotList)) {
				...
				highLotDataHandle.ChartDataHandle(QuartzHandlerType.HU_BEI_KUAI_3.getCode());
				// 修改累计中奖金额
				lotLatestBonusService.updateLatestBonusAmount(lot);
			}else {
				execDate=setUnnormalExecDate();// 稍后再次执行
			}
			logger.info("获取 湖北快3  新数据成功，下次执行时间：" + CommUtil.formatLongDate(execDate));
		} catch (Exception e) {
			logger.info("获取 湖北快3  新数据异常，下次执行时间：" + CommUtil.formatLongDate(execDate));
			e.printStackTrace();
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.HU_BEI_KUAI_3.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.HU_BEI_KUAI_3, info, execDate);
		}
	...
	* @throws
	...
	public String getDescription() {
		return QuartzHandlerType.HU_BEI_KUAI_3.getName();
	}
