...
/**
 * 
* @ClassName: Jsssc_QuartzHandler 
* @Description: 极速时时彩 
* @author A18ccms a18ccms_gmail_com 
* @date 2017年8月29日 下午12:21:06 
*
 */
@Service
public class Jsssc_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Jsssc_QuartzHandler.class);
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10036  极速时时彩  彩票代码
			List<AppLotHistory> lotList=grabResult.getRemenLotHistoryByLotCode(QuartzHandlerType.JI_SU_SHI_SHI_CAI.getCode());
			// 有新数据
			if (AssertValue.isNotNullAndNotEmpty(lotList)) {
				...
				// 新增成功，重新统计
				drawCodeStatisticsFactory.setDrawCodeStatistics(QuartzHandlerType.JI_SU_SHI_SHI_CAI.getCode());
			}else {
				execDate=setUnnormalExecDate();// 稍后再次执行
			}
			logger.debug("获取 极速时时彩  新数据成功，下次执行时间："+CommUtil.formatLongDate(execDate));
		} catch (Exception e) {
			logger.debug("获取 极速时时彩  新数据异常，下次执行时间："+CommUtil.formatLongDate(execDate));
			e.printStackTrace();
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.JI_SU_SHI_SHI_CAI.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.JI_SU_SHI_SHI_CAI, info, execDate);	
		}
...
//	*  	每75秒钟   从00:00:30 到   23:59:15 结束
	...
	* @throws
	...
	public QuartzHandlerType getType() {
		return QuartzHandlerType.JI_SU_SHI_SHI_CAI;
	}

	@Override
	public String getDescription() {
		return QuartzHandlerType.JI_SU_SHI_SHI_CAI.getName();
	}
