...
/**
 * 
* @ClassName: Taiwanbinguo_QuartzHandler 
* @Description: 定时获取  台湾宾果 最新开奖记录执行服务类
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月1日 下午2:04:18 
*
 */
@Service
public class Taiwanbinguo_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Taiwanbinguo_QuartzHandler.class);
	private LotHistoryService lotHistoryService;
	private LotNewService lotNewService;
	private GrabOpenResultService<?> grabResult;

	public Taiwanbinguo_QuartzHandler() {
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10047  台湾宾果  彩票代码
			List<AppLotHistory> lotList=grabResult.getJingwaiLotHistoryByLotCode(QuartzHandlerType.TAI_WANG_BIN_GUO.getCode());
			// 有新数据
			...
			logger.info("获取 台湾宾果 新数据成功，下次执行时间：" + CommUtil.formatLongDate(execDate));
		} catch (Exception e) {
			logger.info("获取 台湾宾果 新数据异常，下次执行时间：" + CommUtil.formatLongDate(execDate));
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.TAI_WANG_BIN_GUO.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.TAI_WANG_BIN_GUO, info, execDate);
		}
	...
	* @throws
	...
	public QuartzHandlerType getType() {
		return QuartzHandlerType.TAI_WANG_BIN_GUO;
	}

	@Override
	public String getDescription() {
		return QuartzHandlerType.TAI_WANG_BIN_GUO.getName();
	}
