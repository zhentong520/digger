...
/**
 * 
* @ClassName: Jiangxi11xuan5_QuartzHandler 
* @Description: 定时获取 江西11选5 最新开奖记录执行服务类
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月1日 下午2:04:18 
*
 */
@Service
public class Jiangxi11xuan5_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Jiangxi11xuan5_QuartzHandler.class);
	...
	public Jiangxi11xuan5_QuartzHandler() {
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10015   江西11选5  彩票代码
			List<AppLotHistory> lotList=grabResult.getGaopinLotHistoryByLotCode(QuartzHandlerType.JIANG_XI_11_XUAN_5.getCode());
			// 有新数据
			if (AssertValue.isNotNullAndNotEmpty(lotList)) {
				...
				logger.info("获取  江西11选5  新数据成功，下次执行时间：" + CommUtil.formatLongDate(execDate));
				// 新增成功，重新统计
				drawCodeStatisticsFactory.setDrawCodeStatistics(QuartzHandlerType.JIANG_XI_11_XUAN_5.getCode());
				// 修改累计中奖金额
			...
			logger.info("获取  江西11选5  新数据异常，下次执行时间：" + CommUtil.formatLongDate(execDate));
			e.printStackTrace();
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.JIANG_XI_11_XUAN_5.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.JIANG_XI_11_XUAN_5, info, execDate);
		}
	...
	* @throws
	...
	public String getDescription() {
		return QuartzHandlerType.JIANG_XI_11_XUAN_5.getName();
	}
