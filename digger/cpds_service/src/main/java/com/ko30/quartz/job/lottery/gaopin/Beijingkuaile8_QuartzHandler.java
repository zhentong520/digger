...
/**
 * 
* @ClassName: Beijingkuaile8_QuartzHandler 
* @Description: 定时获取 北京快乐8 最新开奖记录执行服务类
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月1日 下午2:04:18 
*
 */
@Service
public class Beijingkuaile8_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Beijingkuaile8_QuartzHandler.class);
	...
	public Beijingkuaile8_QuartzHandler() {
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10014  北京快乐8  彩票代码
			List<AppLotHistory> lotList=grabResult.getGaopinLotHistoryByLotCode(QuartzHandlerType.BEI_JING_KUAI_LE_8.getCode());
			// 有新数据
			...
			logger.info("获取 北京快乐8  新数据成功，下次执行时间：" + CommUtil.formatLongDate(execDate)+" 期数："+lot.getPreDrawIssue());
		} catch (Exception e) {
			logger.info("获取 北京快乐8  新数据异常，下次执行时间：" + CommUtil.formatLongDate(execDate));
			e.printStackTrace();
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.BEI_JING_KUAI_LE_8.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.BEI_JING_KUAI_LE_8, info, execDate);
		}
	...
	* @throws
	...
	private Date setNormalExecDate() {
		...
		cal.add(Calendar.MINUTE, 5);// 当前时间 加 5 分钟
		return cal.getTime();
	}
	...
	* @throws
	...
	public String getDescription() {
		return QuartzHandlerType.BEI_JING_KUAI_LE_8.getName();
	}
