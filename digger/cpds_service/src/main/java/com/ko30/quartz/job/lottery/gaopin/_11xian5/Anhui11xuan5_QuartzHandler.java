...
/**
 * 
* @ClassName: Anhui11xuan5_QuartzHandler 
* @Description: 定时获取 安徽11选5 最新开奖记录执行服务类
* @author A18ccms a18ccms_gmail_com 
* @date 2017年9月1日 下午2:04:18 
*
 */
@Service
public class Anhui11xuan5_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Anhui11xuan5_QuartzHandler.class);
	...
	public Anhui11xuan5_QuartzHandler() {
	...
	public Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10017  安徽11选5  彩票代码
			List<AppLotHistory> lotList=grabResult.getGaopinLotHistoryByLotCode(QuartzHandlerType.AN_HUI_11_XUAN_5.getCode());
			// 有新数据
			if (AssertValue.isNotNullAndNotEmpty(lotList)) {
				...
				logger.info("获取 安徽11选5  新数据成功，下次执行时间：" + CommUtil.formatLongDate(execDate));
				// 新增成功，重新统计
				drawCodeStatisticsFactory.setDrawCodeStatistics(QuartzHandlerType.AN_HUI_11_XUAN_5.getCode());
			
				// 修改累计中奖金额
				lotLatestBonusService.updateLatestBonusAmount(lot);
			}else {
				execDate=setUnnormalExecDate();// 稍后再试
			}
		} catch (Exception e) {
			logger.info("获取 安徽11选5  新数据异常，下次执行时间：" + CommUtil.formatLongDate(execDate));
			e.printStackTrace();
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.AN_HUI_11_XUAN_5.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.AN_HUI_11_XUAN_5, info, execDate);
		}
	...
	* @throws
	...
	public QuartzHandlerType getType() {
		return QuartzHandlerType.AN_HUI_11_XUAN_5;
	}

	@Override
	public String getDescription() {
		return QuartzHandlerType.AN_HUI_11_XUAN_5.getName();
	}
