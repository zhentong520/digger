...
/**
 * 
* @ClassName: Tjssc_QuartzHandler 
* @Description: 天津时时彩 
* @author A18ccms a18ccms_gmail_com 
* @date 2017年8月29日 下午2:26:40 
*
 */
@Service
public class Tjssc_QuartzHandler implements QuartzHandler {


	private Logger logger = Logger.getLogger(Tjssc_QuartzHandler.class);
	...
	@Override
	public synchronized Map<String, String> handler(JSONObject data) {
		...
		try {
			// 10003  天津时时彩   彩票代码
			List<AppLotHistory> lotList=grabResult.getRemenLotHistoryByLotCode(QuartzHandlerType.TIAN_JIN_SHI_SHI_CAI.getCode());
			// 有新数据
			if (AssertValue.isNotNullAndNotEmpty(lotList)) {
				...
				// 新增成功，重新统计
				drawCodeStatisticsFactory.setDrawCodeStatistics(QuartzHandlerType.TIAN_JIN_SHI_SHI_CAI.getCode());
			}else {
				execDate=setUnnormalExecDate();// 稍后再次执行
			}
			logger.debug("获取 天津时时彩  新数据成功，下次执行时间："+CommUtil.formatLongDate(execDate));
		} catch (Exception e) {
			logger.debug("获取 天津时时彩  新数据异常，下次执行时间："+CommUtil.formatLongDate(execDate));
		}finally{
			// 为下一次执行作准备
			QuartzParamInfo info = new QuartzParamInfo();
			info.setParamObj(lot);
			info.setExecuteKey(QuartzHandlerType.TIAN_JIN_SHI_SHI_CAI.getType());
			QuartzHandlerFactory factory = SpringUtils.getBean(QuartzHandlerFactory.class);
			factory.executeHandler(QuartzHandlerType.TIAN_JIN_SHI_SHI_CAI, info, execDate);
		}
	...
	*  	每10分钟  从 09:10:30  到    23:00:30  结束
	* @param @return    设定文件 
	...
	* @throws
	...
	public String getDescription() {
		return QuartzHandlerType.TIAN_JIN_SHI_SHI_CAI.getName();
	}
